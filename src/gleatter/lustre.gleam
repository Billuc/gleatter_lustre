//// This module helps to make the bridge between gleatter Routes and Lustre
  ...
  |> lustre_http.send(
    lustre_http.expect_text(fn(body) {
      ...
      |> result.then(fn(value) {
        rreq.route.res_body
        |> body.decode(value)
        |> result.map_error(gleatter_to_http_error)
      })
      |> as_msg
    }),
  )
...
/// Mapper function between GleatterError and HttpError
fn gleatter_to_http_error(err: gleatter.GleatterError) -> lustre_http.HttpError {
  case err {
    gleatter.RouteError(msg) -> lustre_http.OtherError(500, msg)
    gleatter.JsonDecodeError(json_err) -> lustre_http.JsonError(json_err)
  }
}
